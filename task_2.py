...
def preprocess_data(X: pd.DataFrame, y: pd.Series):
    ...
    if y is not None:  # we are in train
        return data.drop(columns=["original_selling_amount"]), data["original_selling_amount"]
    else:  # we are in test
...
if __name__ == '__main__':
    ...
    X_train, y_train = train.loc[:, train.columns != "original_selling_amount"], train["original_selling_amount"]
    X_test, y_test = test.loc[:, test.columns != "original_selling_amount"], test["original_selling_amount"]
